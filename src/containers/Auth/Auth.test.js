...
describe("<Auth />", () => {
    ...
    it("should be invalid if email in wrong", () => {
        expect(wrapper.instance().checkValidity("abce@",{...validation,isEmail: true})).toEqual(false);
    });
    it("should be valid if email in correct", () => {
        expect(wrapper.instance().checkValidity("abce@mail.in",{...validation,isEmail: true})).toEqual(true);
    });
    it("should be invalid if length is less than 6", () => {
        expect(wrapper.instance().checkValidity("12234",{...validation,minLength: 6})).toEqual(false);
    });
    it("should be valid if length is more than 6", () => {
        expect(wrapper.instance().checkValidity("12234234",{...validation,minLength: 6})).toEqual(true);
    });
